...
export async function POST(req: Request) {
  ...
  try {
    await resend.emails.send({
      ...
      subject: `New Support Submission – WebbedLeads`,
      html: `
        <div style="max-width:600px;margin:0 auto;padding:40px;font-family:Helvetica,Arial,sans-serif;background-color:#ffffff;color:#0f172a;border-radius:8px;border:1px solid #e2e8f0;">
          ...
          <h2 style="font-size:20px;margin-bottom:10px;">📥 New Support Submission</h2>
          <p><strong>${name}</strong> submitted a message via the WebbedLeads support form.</p>
      ...
      `,
      text: `
    New Support Submission – WebbedLeads
    
    ${name} submitted a message via the WebbedLeads Support form.
